...
__plugin_meta__ = PluginMetadata(
    ...
    usage=(
        ...
        "使用 --verify 选项开启非默认的成语检查，即猜测的短语必须是成语，如：@我 猜成语 --verify"
    ),
    type="application",
...
parser.add_argument("--verify", action="store_true", help="验证模式，即判断所发送短语是否为成语")
...
async def handle_handle(
    ...
    if not games.get(cid, None):
        game = Handle(*random_idiom(), strict=(handle_config.handle_strict_mode) if handle_config.handle_strict_mode else options.verify)
        games[cid] = game
